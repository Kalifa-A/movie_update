...
export const Footer = () => {
  ...
  return (
    <footer className={`w-full backdrop-blur-2xl border-t mt-auto transition-colors duration-500 ${darkMode ? 'bg-gray-900/60 border-gray-800' : 'bg-white/60 border-gray-100'}`}>
      <div className="max-w-7xl mx-auto px-6 py-12">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-10">
          ...
          <div className="text-center md:text-left">
            ...
            <p className={`text-sm mt-2 max-w-xs font-medium leading-relaxed transition-colors ${darkMode ? 'text-gray-500' : 'text-gray-400'}`}>
          ...
          <nav className="flex flex-wrap justify-center gap-x-8 gap-y-4">
            {footerLinks.map((link) => (
              <NavLink
                key={link.name}
                to={link.to}
                className={`text-sm font-semibold transition-all duration-300 relative group ${darkMode ? 'text-gray-400 hover:text-indigo-400' : 'text-gray-500 hover:text-indigo-600'}`}
              >
                {link.name}
          ...
          <div className="flex flex-col items-center md:items-end gap-3">
             <div className="flex gap-4">
                <span className={`w-9 h-9 flex items-center justify-center rounded-full transition-all cursor-pointer border text-xs font-bold ${darkMode ? 'bg-gray-800 text-gray-400 hover:bg-indigo-900/50 hover:text-indigo-400 border-gray-700' : 'bg-gray-50 text-gray-400 hover:bg-indigo-50 hover:text-indigo-600 border-gray-100'}`}>X</span>
                <span className={`w-9 h-9 flex items-center justify-center rounded-full transition-all cursor-pointer border text-xs font-bold ${darkMode ? 'bg-gray-800 text-gray-400 hover:bg-indigo-900/50 hover:text-indigo-400 border-gray-700' : 'bg-gray-50 text-gray-400 hover:bg-indigo-50 hover:text-indigo-600 border-gray-100'}`}>IG</span>
             </div>
          </div>
        </div>

        {/* Divider Line */}
        <div className={`h-[1px] w-full bg-gradient-to-r from-transparent to-transparent my-10 ${darkMode ? 'via-gray-700' : 'via-gray-200'}`}></div>

        {/* Bottom Bar */}
        <div className="flex flex-col md:flex-row justify-between items-center gap-4">
          <p className={`text-[10px] font-bold uppercase tracking-[0.2em] ${darkMode ? 'text-gray-600' : 'text-gray-400'}`}>
