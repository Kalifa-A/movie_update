...
export default function Watchlist() {
  ...
  if (loading) return (
    <div className={`min-h-screen flex items-center justify-center transition-colors duration-500 ${darkMode ? 'bg-gray-950' : 'bg-[#fbfbfd]'}`}>
  ...
  return (
    <div className={`min-h-screen font-sans pb-20 md:pb-10 pt-24 md:pt-32 transition-colors duration-500 ${darkMode ? 'bg-gray-950 text-gray-100' : 'bg-[#fbfbfd] text-gray-900'}`}>
      <div className="max-w-7xl mx-auto px-6">
        
        {/* Collection Header */}
        <header className="mb-12 md:mb-20">
          <h1 className="text-5xl md:text-5xl font-black tracking-tighter leading-none">
            Your <span className="text-indigo-600">Collection.</span>
          </h1>
          <p className={`mt-6 text-lg md:text-xl font-medium max-w-xl ${darkMode ? 'text-gray-500' : 'text-gray-400'}`}>
            A curated selection of your cinematic favorites, saved for whenever you're ready.
          </p>
        </header>

        {movies.length === 0 ? (
          <div className={`flex flex-col items-center justify-center py-32 rounded-[3rem] border border-dashed shadow-sm ${darkMode ? 'bg-gray-800/50 border-gray-700' : 'bg-white border-gray-200'}`}>
            <div className="text-6xl mb-6 w-40 h-40"><img src={Video} alt="Video Player Icon" /></div>
            <p className={`text-xl font-bold tracking-tight mb-8 ${darkMode ? 'text-gray-400' : 'text-gray-400'}`}>No movies in your vault yet.</p>
            <button 
              onClick={() => navigate('/')}
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6 md:gap-10">
            {movies.map((m) => {
              ...
              return (
                ...
                >
                  {/* Poster Card */}
                  <div className={`relative aspect-[2/3] rounded-[2rem] md:rounded-[2.5rem] overflow-hidden shadow-sm border transition-all duration-700 group-hover:shadow-[0_40px_80px_-15px_rgba(0,0,0,0.15)] group-hover:-translate-y-3 ${darkMode ? 'border-gray-800' : 'border-gray-100'}`}>
                    <img 
                      src={posterUrl} 
                      alt={m.title} 
                  ...
                  <div className="mt-5 px-2">
                    <h3 className={`text-lg md:text-xl font-bold truncate transition-colors group-hover:text-indigo-600 ${darkMode ? 'text-gray-200 group-hover:text-indigo-400' : 'text-gray-900'}`}>
                      {m.title}
                    </h3>
                    <p className={`text-[10px] font-black tracking-[0.2em] uppercase mt-1 ${darkMode ? 'text-gray-600' : 'text-gray-400'}`}>
                      Saved Item
                    </p>
                  </div>
                </div>
