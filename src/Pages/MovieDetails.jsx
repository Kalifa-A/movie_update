    ...
    <div className="max-w-6xl mx-auto px-6 -mt-32 md:-mt-48 relative z-10">
      ...
      <div className="flex flex-col md:flex-row gap-8 md:gap-12 items-start mb-20">
        ...
        <div className="flex-1 w-full pt-0 md:pt-10">
          <div className={`md:backdrop-blur-2xl p-0 md:p-12 rounded-[3.5rem] md:border md:shadow-[0_8px_32px_rgba(0,0,0,0.04)] ${darkMode ? 'md:bg-gray-800/70 md:border-gray-700/40' : 'md:bg-white/70 md:border-white/40'}`}>
            ...
            <div className="flex flex-wrap items-center gap-2 md:gap-3 mb-6">
              {movie.genres?.map(g => (
                <span
                  key={g.id}
                  className={`px-3 md:px-4 py-1.5 text-[9px] md:text-[10px] font-extrabold rounded-full tracking-widest uppercase border ${darkMode ? 'bg-indigo-900/30 text-indigo-400 border-indigo-800/50' : 'bg-indigo-50/50 text-indigo-600 border-indigo-100/50'}`}
                >
                  {g.name}
                </span>
              ))}
            ...
            <div className="flex items-center gap-4 md:gap-8 mb-10 text-sm md:text-lg font-semibold">
              <div className="flex items-center gap-2.5">
                ...
                <span>
                  ...
                  <span className={`font-medium ${darkMode ? 'text-gray-500' : 'text-gray-400'}`}> / 10</span>
                </span>
              </div>
...
<div className="mb-12">
  ...
  <div className="flex gap-6 overflow-x-auto pb-6 scrollbar-hide snap-x">
    {cast && cast.map((person) => (
      <div key={person.id} className="text-center shrink-0 group snap-start">
        ...
        {/* Name & Character */}
        <p className={`text-[11px] font-black w-24 truncate leading-tight ${darkMode ? 'text-gray-200' : 'text-gray-900'}`}>
          {person.name}
        </p>
        <p className={`text-[9px] font-bold truncate w-24 mt-1 uppercase tracking-tighter ${darkMode ? 'text-gray-500' : 'text-gray-400'}`}>
          {person.character}
        </p>
      </div>
    ))}
            ...
            <div className="hidden md:flex items-center gap-5 mt-4">
              ...
              >
                <div className="w-10 h-10 flex items-center justify-center bg-transparent rounded-full transition-transform duration-500 group-hover:rotate-[360deg]">
                  <img src={play} alt="play" className={`w-full h-full object-contain ${darkMode ? 'invert' : ''}`} />
                </div>
        ...
        <div className="flex overflow-x-auto md:grid md:grid-cols-4 lg:grid-cols-5 gap-6 md:gap-10 pb-10 scrollbar-hide snap-x snap-mandatory px-2 md:px-0">
          {related.map((m) => {
            ...
            return (
              ...
              >
                <div className={`relative aspect-[2/3] rounded-[2.2rem] md:rounded-[2.5rem] overflow-hidden mb-4 shadow-sm border transition-all duration-700 hover:shadow-xl hover:-translate-y-3 ${darkMode ? 'border-gray-800' : 'border-gray-100'}`}>
                  <img
                    src={relatedPoster}
                    alt={m.title}
                ...
                <h3 className={`text-lg font-bold truncate px-2 transition-colors ${darkMode ? 'group-hover:text-indigo-400' : 'group-hover:text-indigo-600'}`}>
                  {m.title}
                </h3>
                <p className={`text-[10px] font-bold uppercase tracking-widest px-2 mt-1 ${darkMode ? 'text-gray-600' : 'text-gray-400'}`}>
  ...
  <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-8">
    {/* Ultra-Blur Backdrop */}
    <div
      className={`absolute inset-0 backdrop-blur-[40px] ${darkMode ? 'bg-gray-950/85' : 'bg-[#fbfbfd]/85'}`}
    ...
    <div className="relative w-full max-w-6xl aspect-video z-10 group">
      ...
      <div className="absolute -bottom-16 left-1/2 -translate-x-1/2 w-full flex justify-center pb-4">
        <div className={`px-6 py-2.5 backdrop-blur-2xl border rounded-full shadow-lg flex items-center gap-3 ${darkMode ? 'bg-gray-800/40 border-gray-700/40' : 'bg-white/40 border-white/40'}`}>
          ...
          <div className="md:backdrop-blur-2xl md:bg-white/70 p-0 md:p-12 rounded-[3.5rem] md:border md:border-white/40 md:shadow-[0_8px_32px_rgba(0,0,0,0.04)]">
            ...
            <p className="text-gray-500 leading-relaxed text-lg md:text-xl font-medium mb-12 max-w-2xl">
              {movie.overview}
                        ...
                        {/* TOP CAST SECTION: Horizontal Swipe */}
{/* TOP CAST SECTION: Updated with Instagram IDs */}
<div className="mb-12">
  <div className="flex items-center justify-between mb-8">
    <h3 className="text-xl font-bold tracking-tight">Top Cast</h3>
  ...
  <div className="flex gap-6 overflow-x-auto pb-6 scrollbar-hide snap-x">
    {cast && cast.map((person) => (
      <div key={person.id} className="text-center shrink-0 group snap-start relative">
        
        {/* Avatar Circle with Social Overlay */}
        <div className="relative w-16 h-16 md:w-20 md:h-20 mx-auto mb-4">
          <div className="w-full h-full rounded-full overflow-hidden shadow-md border-2 border-white transition-all duration-500 group-hover:scale-110 group-hover:shadow-indigo-100 group-hover:border-indigo-50">
            <img 
              src={person.profile_path 
                ? `https://image.tmdb.org/t/p/w200${person.profile_path}` 
                : 'https://via.placeholder.com/200?text=N/A'} 
          ...
          {/* INSTAGRAM OVERLAY BUTTON - Only shows on hover */}
          <a 
            href={`https://www.instagram.com/${person.name.toLowerCase().replace(/\s+/g, '')}`} 
            target="_blank" 
            rel="noopener noreferrer"
            className="absolute inset-0 flex items-center justify-center bg-indigo-600/60 backdrop-blur-[2px] rounded-full opacity-0 group-hover:opacity-100 transition-all duration-300 transform scale-50 group-hover:scale-100"
          >
